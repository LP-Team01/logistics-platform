...
CREATE INDEX idx_order_receiver_company_id ON p_orders (receiver_company_id);
CREATE INDEX idx_order_status ON p_orders (status);
CREATE INDEX idx_order_deleted_at ON p_orders (deleted_at);
...
CREATE TABLE p_order_items (
    ...
    CONSTRAINT fk_order_items_order FOREIGN KEY (order_id) REFERENCES p_orders(order_id),
